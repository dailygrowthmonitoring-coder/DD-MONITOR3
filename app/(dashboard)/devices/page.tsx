...
function DomainCard({ device }: { device: DeviceOverviewItem }) {
  ...
  return (
    <Link href={`/devices/${device.id}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
      <div style={{
        background: 'var(--bg2)', border: '1px solid var(--line)', borderRadius: 'var(--r)',
        padding: 14, cursor: 'pointer',
        borderLeft: `2px solid ${LEFT_BORDER[cls]}`,
      }}>
        ...
        <div style={{ height: 3, background: 'var(--bg4)', borderRadius: 2, overflow: 'hidden', marginBottom: 8 }}>
          <div style={{ height: 3, borderRadius: 2, width: `${pct}%`, background: FILL_COLOR[cls] }} />
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
          <div style={{ background: 'var(--bg3)', border: '1px solid var(--line)', borderRadius: 2, padding: '8px 10px' }}>
            <div style={{ fontSize: 16, fontWeight: 600, fontFamily: 'var(--font-geist-mono),monospace', letterSpacing: '-1px', color: device.active_alerts_critical > 0 ? 'var(--red)' : 'var(--green)' }}>
              {device.active_alerts_critical}
            </div>
          ...
          <div style={{ background: 'var(--bg3)', border: '1px solid var(--line)', borderRadius: 2, padding: '8px 10px' }}>
            <div style={{ fontSize: 16, fontWeight: 600, fontFamily: 'var(--font-geist-mono),monospace', letterSpacing: '-1px', color: device.active_alerts_warning > 0 ? 'var(--amber)' : 'var(--text2)' }}>
              {device.active_alerts_warning}
            </div>
            <div style={{ fontSize: 9, fontFamily: 'var(--font-geist-mono),monospace', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.5px', marginTop: 2 }}>
              warning
            </div>
...
export default function DomainsPage() {
  ...
  return (
    <div className="anim-fadein">
      <div style={{ padding: '20px 24px 0', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          ...
          <div style={{ fontSize: 11.5, color: 'var(--muted)', fontFamily: 'var(--font-geist-mono),monospace', marginTop: 3 }}>
            {devices.length} systems · all reachable
          </div>
        </div>
      ...
      {error && <div style={{ padding: '0 24px 20px' }}><ErrorState message="Failed to load devices" onRetry={() => void refresh()} /></div>}

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, padding: '0 24px 20px' }}>
        {isLoading && Array.from({ length: 6 }).map((_, i) => (
          <div key={i} style={{ background: 'var(--bg2)', border: '1px solid var(--line)', borderRadius: 'var(--r)', padding: 14, height: 140 }} />
        ))}
        ...
        {!isLoading && devices.length === 0 && !error && (
          <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '60px 0', color: 'var(--muted)', fontSize: 13 }}>
            No domains registered yet. Reports will appear here once ingested.
          </div>
        )}
      </div>
    </div>
